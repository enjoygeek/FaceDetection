...
public class OpenCVProcessor {
	...
	void detectAndDisplay(Mat image) {
		...
		// -- Detect faces
		faceDetector.detectMultiScale(frame_gray, faces, 1.1, 2, 0, new Size(3,
				3), new Size(3000, 3000));
		System.out.println("FACES: " + faces.size());
		for (Rect rect : faces.toArray()) {
			Point center = new Point(rect.x + rect.width * 0.5, rect.y
					+ rect.height * 0.5);
			FaceDetection faceDetection = new FaceDetection();
			faceDetection.setBoundingBox(new Rectangle2D.Double(
					rect.width * 0.5, rect.height * 0.5, rect.width,
					rect.height));
			org.opencv.imgproc.Imgproc.ellipse(image, center, new Size(
					rect.width * 0.5, rect.height * 0.5), 0, 0, 360,
					new Scalar(255, 0, 255), 4, 8, 0);
			Mat faceROI = frame_gray;
			MatOfRect eyes = new MatOfRect();

			// -- In each face, detect eyes
			eyeDetector.detectMultiScale(faceROI, eyes, 1.1, 2, 0, new Size(30,
					30), new Size(300, 300));
			System.out.println("EYES: " + eyes.size());
			for (Rect eye : eyes.toArray()) {
				System.out.println(eye.x + "-" + eye.y + "-" + eye.width + "-"
						+ eye.height);
				Point center_eye = new Point(eye.x + eye.width * 0.5, eye.y
						+ eye.height * 0.5);
				int radius = (int) Math
						.round((eye.width + eye.height) * 0.0005);

				org.opencv.imgproc.Imgproc.circle(image, center_eye, radius,
	...
	public List<ProcessResult> process(String imagePath) {
		...
		for (int j = 0; j < clasificadores.size(); j++) {
			...
			for (Rect rect : faceDetections.toArray()) {
				...
				faceDetection.setBoundingBox(new Rectangle2D.Double(rect.x,
